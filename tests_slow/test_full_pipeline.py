...
async def test_async_job_flow():
    ...
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        resp = await client.post("/ask?async_job=true", json=payload, headers=AUTH_HEADERS)
        assert resp.status_code == 202
        job_id = resp.json()["job_id"]

        status_resp = await client.get(f"/jobs/{job_id}", headers=AUTH_HEADERS)
...
async def test_metrics_endpoint():
    ...
    transport = httpx.ASGITransport(app=app)
    app.state.admin_rate_limiter = None
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
