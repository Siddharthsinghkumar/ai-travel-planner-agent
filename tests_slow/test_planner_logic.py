...
@pytest.mark.asyncio
async def test_action_intent_hold_uses_prefetched_selection_without_planning(monkeypatch):
    async def should_not_plan(**_kwargs):
        raise AssertionError("_plan_trip_internal should not run when prefetched action selection is complete")

    async def fake_hold_booking_safe(**_kwargs):
        return {"id": 77, "status": "HELD", "checkout_ready": False, "hold_outcome": "held_local_only"}

    monkeypatch.setattr(planner_agent, "_plan_trip_internal", should_not_plan)
    monkeypatch.setattr(planner_agent, "_hold_booking_safe", fake_hold_booking_safe)

    result = await planner_agent.plan_trip(
        user_query="hold this flight",
        stream=False,
        origin="DEL",
        destination="BOM",
        date="2026-06-12",
    ...
    assert result["action"] == "hold_booking"
    assert result["success"] is True
    assert result["selection_source"] == "prefetched_flight"
...
def test_parse_intent_cabin_bag_phrase_sets_hand_baggage_pref():
    intent = parse_intent("Delhi Mumbai trip on 2026-05-03, just cabin bag")
    assert intent.baggage_pref == "hand"
    assert intent.origin_iata == "DEL"
    assert intent.destination_iata == "BOM"
...
def test_generate_deterministic_summary_frontloads_relaxed_preference_disclosures():
    flight = Flight(
        airline="TestAir",
        flight_no="TA404",
    ...
    text = planner_agent.generate_deterministic_summary(
        flight,
        {"condition": "Clear", "temperature_c": 27},
        "default source order",
        error="timed out",
        location="BOM",
        intent=intent,
        warnings=[
            "No exact flights were available for preferred airline(s) indigo; relaxed airline preference.",
            "No flights matched the hand baggage preference; relaxed baggage constraint.",
        ],
    )
    text_low = text.lower()
    assert text_low.startswith("no matching flights were found for your preferred airline")
    assert "no hand-baggage-only flight was available" in text_low
    assert "i recommend testair ta404" in text_low
...
def test_infer_route_pair_from_query_handles_cabin_bag_phrase():
    origin, destination, trace = planner_agent._infer_route_pair_from_query(
        "Delhi Mumbai trip on 2026-05-03, just cabin bag"
    )
    assert origin == "DEL"
    assert destination == "BOM"
    assert trace.get("source") in {
        "deterministic_adjacent_city_pair",
        "resolver_phrase_pair",
        "resolver_phrase_pair_from_to_fallback",
    }
